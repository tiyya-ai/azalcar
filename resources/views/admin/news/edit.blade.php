...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="row g-4 mb-4">
        <div class="col-sm-6 col-xl-3">
            <h4 class="fw-bold py-3 mb-4">
                <span class="text-muted fw-light">Management / News /</span> Edit Article
            </h4>
        </div>
    ...
    <!-- Basic Layout -->
    <form id="editNewsForm" action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="col-xl-8">
                <div class="card mb-4 shadow-sm">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 fw-bold text-primary"><i class="bx bx-news me-2"></i>Edit Article Content</h5>
                        <small class="text-muted float-end">ID: {{ $news->id }}</small>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4 p-3 bg-lighter rounded">
                            ...
                            @else
                                <span class="badge bg-warning">DRAFT / SCHEDULED</span>
                            @endif
                        </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <div class="input-group input-group-merge">
                                    ...
                                    <input type="text" class="form-control" id="title" name="title" value="{{ old('title', $news->title) }}" placeholder="e.g. 2026 Model X Review" required />
                                </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control" id="excerpt" name="excerpt" rows="2" maxlength="500" placeholder="Brief summary for listing cards...">{{ old('excerpt', $news->excerpt) }}</textarea>
                            ...
                            <div class="col-12">
                                ...
                                <div id="quill-editor" style="height: 400px;">
                                    @php echo old('content', $news->content); @endphp
                                </div>
                        ...
                        <div class="mt-5 pt-3 border-top d-flex justify-content-between">
                            <button type="button" class="btn btn-label-danger" onclick="if(confirm('Delete this article?')) document.getElementById('delete-form').submit();">
                                <i class="bx bx-trash me-1"></i> Delete
                            </button>
                            <button type="submit" class="btn btn-primary btn-lg">
                                <i class="bx bx-save me-1"></i> Update Article
                            </button>
                        </div>
                    </div>
                </div>
                 ...
                 <div class="card mb-4 shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-center bg-lighter border rounded mb-3 p-2" style="height: 200px; position: relative; overflow: hidden;">
                             @if($news->image)
                                <img src="{{ asset('storage/' . $news->image) }}" alt="Current Image" class="img-fluid rounded" id="imagePreview" style="max-height: 100%; width: auto; object-fit: contain;">
                            @else
                    ...
                    <div class="card-body">
                         ...
                         <div class="mb-3">
                            ...
                            <select class="form-select text-uppercase fw-bold" id="status" name="status" required>
                                <option value="published" {{ old('status', $news->status) == 'published' ? 'selected' : '' }} class="text-success">Published</option>
                                <option value="draft" {{ old('status', $news->status) == 'draft' ? 'selected' : '' }} class="text-secondary">Draft</option>
                                <option value="archived" {{ old('status', $news->status) == 'archived' ? 'selected' : '' }} class="text-warning">Archived</option>
                            </select>
                        </div>
                         ...
                         <div class="mb-3">
                            ...
                            <select class="form-select" id="category" name="category" required>
                                ...
                                <option value="Guide" {{ old('category', $news->category) == 'Guide' ? 'selected' : '' }}>Guide</option>
                                <option value="Video" {{ old('category', $news->category) == 'Video' ? 'selected' : '' }}>Video</option>
    ...
    <form id="delete-form" action="{{ route('admin.news.destroy', $news) }}" method="POST" class="d-none">
        @csrf
        @method('DELETE')
    </form>
</div>
